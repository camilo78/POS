...
class SaleController extends Controller
{
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
           ...
           //dd($correlative);
            ...
            $lims_warehouse_list = Warehouse::where('is_active', true)->get();
            $lims_period_list = Period::where('status', 1)->get();
            $lims_biller_list = Biller::where('is_active', true)->get();
            $lims_reward_point_setting_data = RewardPointSetting::latest()->first();
            $lims_tax_list = Tax::where('is_active', true)->get();
            ...
            $flag = 0;
            return view('sale.pos', compact('all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_period_list', 'lims_reward_point_setting_data', 'period_invoice_data', 'lims_product_list', 'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_brand_list', 'correlative','lims_category_list', 'recent_sale', 'recent_draft', 'lims_coupon_list', 'flag','period_id'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function genInvoice($id)
    {
        ...
        $balance = number_format($lims_sale_data->grand_total - $lims_sale_data->paid_amount, 2);
        ...
        return view('sale.invoice', compact('lims_sale_data','balance','lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords'));
    }
